...
void initialize_lru(struct ssd_info *ssd) {
    ...
    //the capacity of nvm lru list is max page number of nvm
    ...
    ssd->dram->map->lru_tail->post = NULL;
    ssd->dram->nvm_map->lru_head->pre = NULL;
    ssd->dram->nvm_map->lru_head->post = ssd->dram->nvm_map->lru_tail;
    ssd->dram->nvm_map->lru_tail->pre = ssd->dram->nvm_map->lru_head;
    ssd->dram->nvm_map->lru_tail->post = NULL;
}
